...
public class SubTaskDaoImpl extends AbstractDao implements SubTaskDao {
	...
	public Set<SubTask> getSubTasksByTaskId(Long taskId) {
		...
		result.addAll(em.createQuery("SELECT st FROM SubTask st WHERE st.parentTask.id = :taskId").setParameter("taskId", taskId).getResultList());
		return result;
	}
	...
	public Set<SubTask> getSubTasksByUser(Long userId) {
		LinkedHashSet<SubTask> result = new LinkedHashSet<SubTask>();
		result.addAll(em.createQuery("SELECT st FROM SubTask st WHERE st.lastUserActivity.id = :lastUserActivity AND st.taken = true")
