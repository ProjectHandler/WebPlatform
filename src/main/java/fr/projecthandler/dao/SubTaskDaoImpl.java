...
public class SubTaskDaoImpl extends AbstractDao implements SubTaskDao {
	...
	public void deleteSubTaskById(Long id) {
		em.createQuery("DELETE FROM SubTask st WHERE st.id =:id")
	...
	public SubTask findSubTaskById(Long subTaskId) {
		return (SubTask) Utilities.getSingleResultOrNull(em.createQuery(
				"SELECT st FROM SubTask st WHERE st.id = :subTaskId")
	...
	public Set<SubTask> getSubTasksByTaskIdAndFetchUserAndTask(Long taskId) {
		...
		result.addAll(em
				.createQuery(
						"SELECT st FROM SubTask st LEFT JOIN FETCH st.lastUserActivity u LEFT JOIN FETCH st.parentTask t "
								+ "WHERE st.parentTask.id = :taskId")
	...
	public Set<SubTask> getSubTasksUnplannedByUser(Long userId) {
		...
		result.addAll(em
				.createQuery(
						"SELECT st FROM SubTask st JOIN st.lastUserActivity WHERE st.lastUserActivity.id = :lastUserActivity AND st.startingDate IS NULL AND st.endingDate IS NULL AND st.taken = true")
				.setParameter("lastUserActivity", userId).getResultList());
		return result;
	}
	...
	public Set<SubTask> getSubTasksPlannedByUser(Long userId) {
		...
		result.addAll(em
				.createQuery(
						"SELECT st FROM SubTask st JOIN st.lastUserActivity WHERE st.lastUserActivity.id = :lastUserActivity AND st.startingDate IS NOT NULL AND st.endingDate IS NOT NULL AND st.taken = true")
				.setParameter("lastUserActivity", userId).getResultList());
		return result;
	}
