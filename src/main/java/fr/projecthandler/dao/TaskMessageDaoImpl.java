...
public class TaskMessageDaoImpl extends AbstractDao implements TaskMessageDao {
	...
	public void deleteTaskMessageById(Long taskMessageId) {
		em.createQuery("DELETE FROM TaskMessage tm WHERE tm.id = :taskMessageId")
		.setParameter("taskMessageId", taskMessageId).executeUpdate();
	}

	@Override
	@Transactional
	public void deleteTaskMessagesByIds(List<Long> taskMessageIds) {
		em.createQuery("DELETE FROM TaskMessage tm WHERE tm.id IN (:taskMessageIds)")
		.setParameter("taskMessageIds", taskMessageIds).executeUpdate();
	}

	@Override
	public TaskMessage findTaskMessageById(Long taskMessageId) {
		return (TaskMessage) Utilities.getSingleResultOrNull(em.createQuery(
				"SELECT tm FROM TaskMessage tm WHERE tm.id = :taskMessageId").setParameter(
	...
	public Set<TaskMessage> getTaskMessagesByTaskId(Long taskId) {
		...
		result.addAll(em.createQuery(
				"SELECT tms FROM TaskMessage tms WHERE tms.task.id = :taskId ORDER BY tms.updateDate DESC")
