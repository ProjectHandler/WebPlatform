...
public class TaskDaoImpl extends AbstractDao implements TaskDao {
	...
	public Set<Task> getYesterdayTasksByUser(Long userId) {
		Set<Task> result = new HashSet<Task>();
		result.addAll(em.createQuery("SELECT t FROM Task t JOIN t.users AS u WHERE t.level = 2 AND t.startingDate = '" + DateHelper.getYesterdayDate("yyyy-MM-dd") + "' AND :userId IN (u.id)").setParameter("userId", userId).getResultList());
		return result;
	}
	...
	public Set<Task> getTodayTasksByUser(Long userId) {
		Set<Task> result = new HashSet<Task>();
		result.addAll(em.createQuery("SELECT t FROM Task t JOIN t.users AS u WHERE t.level = 2 AND t.startingDate = '" + DateHelper.getTodayDate("yyyy-MM-dd") + "' AND :userId IN (u.id)").setParameter("userId", userId).getResultList());
		return result;
	}
	...
	public Set<Task> getTomorrowTasksByUser(Long userId) {
		Set<Task> result = new HashSet<Task>();
		result.addAll(em.createQuery("SELECT t FROM Task t JOIN t.users AS u WHERE t.level = 2 AND t.startingDate = '" + DateHelper.getTomorrowDate("yyyy-MM-dd") + "' AND :userId IN (u.id)").setParameter("userId", userId).getResultList());
		return result;
	}
}
