...
public class EventRestController {
	...
	public @ResponseBody ResponseEntity<String> get(@PathVariable Long id) {
		...
		try {
			objectMapper.writeValue(stringEmp, event);
	...
	public @ResponseBody ResponseEntity<String> getEventsByUser(
			@PathVariable Long id) {
		Set<Event> events = eventService.getEventsByUser(id);

		ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.configure(SerializationFeature.INDENT_OUTPUT, true);
		StringWriter stringEmp = new StringWriter();
		try {
			objectMapper.writeValue(stringEmp, events);
		} catch (JsonGenerationException e) {
			e.printStackTrace();
		} catch (JsonMappingException e) {
			e.printStackTrace();
